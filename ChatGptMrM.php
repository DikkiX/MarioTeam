...
if (($_SERVER["REQUEST_METHOD"] == "POST") && (isset($_POST['user']))) {
  ...
  if ($user != 'wacht op 2de bericht') {
    ...
    if ($debug || preg_match('/^2001:1c00:bd0e:7700:/', $_SERVER['REMOTE_ADDR']))
      echo "<div class='chat-message system'>
  ...
  if ($debug) {
    ...
    $dbg .= "assistant0: " . (string) $assistant0 . "\n";
    $dbg .= "Aankoop=" . (preg_match("/Aankoop/i", $assistant0) ? "1" : "0") . " ";
    $dbg .= "Zending=" . (preg_match("/Zending/i", $assistant0) ? "1" : "0") . " ";
    $dbg .= "Inkoop=" . (preg_match("/Inkoop/i", $assistant0) ? "1" : "0") . " ";
    $dbg .= "Service=" . (preg_match("/Service/i", $assistant0) ? "1" : "0") . " ";
    $dbg .= "Loyaliteit=" . (preg_match("/Loyaliteit/i", $assistant0) ? "1" : "0") . " ";
    $dbg .= "Persoonlijk=" . (preg_match("/Persoonlijk/i", $assistant0) ? "1" : "0") . "\n";
    $dbg .= "Platform: " . (string) $platform . "\n";
    echo "<div class='chat-message system'><p style='white-space:pre-wrap;'>" . htmlspecialchars($dbg, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "<span class='message-time'>" . date("H:i") . "</span></p></div>";
  }
